...
#if defined(NDEBUG)
# error "Namoto cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6ae5064772a0f5d1aec4f6f79d79a315ec9182942fad8ed4c58d4e925f4039bf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Namoto: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Namoto Signed Message:\n";
...
{
    // Namoto: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1000000 blocks, which will occur approximately every 12 years
    nSubsidy >>= (nHeight / 1000000); // Namoto: 1MM blocks in 12 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 29.4 * 24 * 60 * 60; // Namoto: 29.4 days
static const int64 nTargetSpacing = 6.3 * 60; // Namoto: 6.3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Namoto: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Namoto: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x161530c30114cec18dd3b26b4aec18cee64fdb86d0df63debbe712e7ba202339");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Financial Times 15/Jan/2023 China's rapid reopening will stir the global economy";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 16 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04373148656782554934cc1872c55b7f7dfedea71dcdd9cdeb0d00ddb08bf92116fca309ec8c13bba28a804044df956ab9ca7809f7b9f5b0ef414d32e69fd4248f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1673789688;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087153050;

        if (fTestNet)
        {
            block.nTime    = 1673789622;
            block.nNonce   = 386057881;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb4353c23eea33264a111b91c8677ac0f0d1bb9f833a8768cdc22811967b9ac8f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf7, 0xc4, 0xbd, 0xd4 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NamotoMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NamotoMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NamotoMiner(CWallet *pwallet)
{
    printf("NamotoMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("namoto-miner");
    ...
    { loop 
        { while (vNodes.empty())
                ...
                //
            {
                ...
                printf("Running NamotoMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NamotoMiner, pwallet));
}
